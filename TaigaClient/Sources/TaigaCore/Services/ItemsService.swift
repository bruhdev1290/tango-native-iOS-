...
public actor ItemsService {
    ...
    public func createTask(projectId: Int, subject: String, userStoryId: Int?, assignedTo: Int?, token: AuthToken) async throws -> Task {
        try await api.createTask(projectId: projectId, subject: subject, userStoryId: userStoryId, assignedTo: assignedTo, token: token)
    }
    ...
    public func updateIssue(id: Int, subject: String, status: Int?, assignedTo: Int?, token: AuthToken) async throws -> Issue {
        try await api.updateIssue(id: id, subject: subject, status: status, assignedTo: assignedTo, token: token)
    }
}
